...
<script>
	...
	function sendit() {
		...
		xhr.onload = function() {
			if (xhr.status === 200 || xhr.status === 201) {
				...
				if (response.result == 'success') {
					alert("주소지 저장완료");
					location.href='/shopping/delivery.do';
				}
			} else {
		...
		formData.del_address = address;
		formData.del_detailaddress = detailAddress;
		formData.del_postcode =  postCode;
		formData.del_phone = phone.replace(/^(\d{2,3})(\d{3,4})(\d{4})$/, '$1-$2-$3');
...
<body>
	...
	<form name="regform" method="post">
		...
		<p>
			<input type="button" value="주소 추가" onclick="sendit()" alter="주소가 추가되었습니다. "/>
	</form>
</body>
</html>
