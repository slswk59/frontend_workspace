			...
			<div class="cart_group">
				<div class="cart_goods">
					<div class="cart_goodslist">
						<ul class="cart_goodslist">
							...
							<form
								action="${pageContext.request.contextPath}/shopping/delete.do"
								method="post" class="cart_delete_form">
								<c:forEach items="${aList}" var="dto">
									<li class="cart_goodslist"><a
										class="cart_goodslist_image_a"> <span
										...
										<div class="cart_gooslist_goodsname_group">
											<a class="cart_gooslist_goodsname_group"> <span
												style="display: none;">${dto.productDTO.pr_key}</span> <input
												style="display: none;" class="cart_key" type="number"
												name="cart_key" value="${dto.cart_key}" />
										...
										<div class="goods_choice_countdiv_countplus_cart">
											...
											<input type="hidden" class="goods_dcprice_hidden"
												value="${dto.productDTO.pr_dcprice}">
											<button class="goods_minusbutton cart_buttons" type="button"
												aria-label="수량내리기" style="cursor: pointer"></button>
											<div class="goods_countnumber cart_buttons"
												value="${dto.cart_count}">${dto.cart_count}</div>
											<button class="goods_plusbutton cart_buttons" type="button"
												aria-label="수량올리기" style="cursor: pointer"
												data-cart_price="${dto.cart_price}"></button>
											<div class="cart_goods_price">
												<span aria-label="판매 가격" data-testid="product-price"
													class="cart_goods_dcprice" value="${dto.cart_price}">${dto.cart_price}</span>
												<div class="cart_buttons_won">원</div>
											</div>
										</div>
										<button class="cart_cancle_button" type="button"
											data-testid="delete" data-cart_key="${dto.cart_key}">
				...
				<div class="cart_delivery_dest_group">
					<form class="order_form"
						action="${pageContext.request.contextPath}/shopping/insertOrder.do"
						method="post">
						<div class="cart_delivery_dest">
							<div class="cart_destination">
								...
								<c:forEach items="${bList}" var="deli">
									<div class="cart_destination_address_group">
										...
										<p class="cart_destination_address" id="address">${deli.del_address}
											${deli.del_detailaddress}</p>
										<button class="cart_destination_change" type="button"
											height="36" radius="3" style="cursor: pointer"
											onclick="window.open('http://localhost:8090/shopping/delivery.do', 'btn-add-delivery', 'width=900, height=500')">
											새 배송지 추가</button>
									</div>
								</c:forEach>
							</div>
							<div class="cart_price_group">
								<div class="cart_price_group_under">
									<span class="cart_price_title">결제예정금액</span> <span
										class="cart_price_group_count_span" style="display: none;">0</span>
									<span class="cart_price_group_value"> <strong
										class="cart_price_group_value_strong" id="sum_p_price">0000</strong>
										<span class="cart_price_group_value_won">원</span></span>
								</div>
							</div>
							<div class="cart_order_button_div">
								<button class="cart_order_button" type="button" height="56"
									radius="3">
